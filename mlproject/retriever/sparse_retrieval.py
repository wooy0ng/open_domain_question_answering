...
class SparseRetrieval():
    def __init__(
        ...
        self.contexts = list(dict.fromkeys([v['text'] for v in wiki.values()]))     # 문장 중복 제거
        ...
        self.p_embedding = None     # get_sparse_embedding() 함수가 생성됨
    ...
    def retrieve(
        ...
        assert self.p_embedding is not None, "get_sparse_embedding()을 먼저 수행해야 합니다."
        ...
        if isinstance(query_or_dataset, str):
            ...
            return (doc_scores, [self.contexts[doc_indices[idx]] for idx in range(top_k)])
    ...
    def get_relevant_doc(self, query: str, k: Optional[int]=1) -> Tuple[List, List]:
        ...
        result = query_vector * self.p_embedding.T
    ...
    def get_relevant_doc_bulk(self, queries: List, k: Optional[int]=1):
        ...
        spinner.start(desc='inner product')
        result = query_vectors * self.p_embedding.T     # inner product (similarity)
        if not isinstance(result, np.ndarray):
            result = result.toarray()
        spinner.stop()
        
        doc_scores, doc_indices = [], []
        for idx, _ in enumerate(tqdm(result, desc='[+] calculate doc_scores & doc_indices', total=result.shape[0])):
